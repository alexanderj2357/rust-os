...
fn main() {
    ...
    let mut settings = serial.read_settings().expect("Couldn't initiate settings");
    settings.set_baud_rate(opt.baud_rate).expect("Couldn't initialise baud rate");
    settings.set_char_size(opt.char_width);
    settings.set_flow_control(opt.flow_control);
    settings.set_stop_bits(opt.stop_bits);

    serial.write_settings(&settings).expect("Couldn't write settings");

    serial.set_timeout(Duration::from_secs(opt.timeout)).expect("Failed to set timeout");

    let mut reader: Box<io::Read> = match opt.input {
        Some(filepath) => Box::new(BufReader::new(File::open(filepath).expect("File not found"))),
        None => Box::new(BufReader::new(io::stdin()))
    };

    if opt.raw {
        let bytes = io::copy(&mut reader, &mut serial).expect("Copy failed");
        println!("Number of bytes written: {}", bytes);
    } else {
        let bytes = Xmodem::transmit_with_progress(reader, serial, |progress| {
            match progress {
                Progress::Packet(i) => {
                    println!("Read packet {}", i);
                },
                Progress::Waiting => {
                    println!("Waiting");
                },
