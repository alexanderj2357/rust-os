...
const XMODEM_PACKET_SIZE: usize = 128;
...
impl<T: io::Read + io::Write> Xmodem<T> {
    ...
    fn expect_byte(&mut self, byte: u8, expected: &'static str) -> io::Result<u8> {
        ...
        match source_byte == byte {
            true => Ok(source_byte),
            false => Err(io::Error::new(io::ErrorKind::InvalidData, expected))
        }
    }
    ...
    pub fn read_packet(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let header_byte: u8 = self.read_byte(true)?;
        ...
        match header_byte {
            ...
            _ => {
                ...
                "Invalid data received, expected SOH or EOT"));
            }
        }
    }
    ...
    pub fn write_packet(&mut self, buf: &[u8]) -> io::Result<usize> {
        if !self.started {
            ...
            self.expect_byte(NAK, "Expected NAK to indicate start")?;
            self.started = true;
        }
        ...
        match buf.len() {
            XMODEM_PACKET_SIZE => {
                ...
                self.expect_byte_or_cancel(ACK, "Expected ACK to confirm receipt of packet")?;
